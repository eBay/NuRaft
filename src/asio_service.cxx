...
public:
    ...
    static void write(bool is_ssl, ssl_socket& _ssl_socket, asio::ip::tcp::socket& tcp_socket, const BB& buffer,
                      FF func) {
        if (is_ssl)
    ...
    template < typename BB, typename FF >
    static void read(bool is_ssl, ssl_socket& _ssl_socket, asio::ip::tcp::socket& tcp_socket, const BB& buffer,
                     FF func) {
        if (is_ssl)
...
public:
    void prepare_handshake() {
        ...
        p_in("session %" PRIu64 " got connection from %s:%u (as a server)", session_id_, cached_address_.c_str(),
             cached_port_);
    ...
    void handle_handshake(std::shared_ptr< rpc_session > self, const ERROR_CODE& err) {
        if (!err) {
            ...
            p_er("session %" PRIu64 " handshake with %s:%u failed: error %d, %s", session_id_, cached_address_.c_str(),
            ...
            timer->async_wait([self, this, timer](const ERROR_CODE& err) -> void {
                if (err) {
                    p_er("session %" PRIu64 " error happend during "
                         "async wait: %d, %s",
                         session_id_, err.value(), err.message().c_str());
                }
                this->stop();
            });
        }
                 ...
                 [this, self](const ERROR_CODE& err, size_t) -> void {
                     if (err) {
                         p_er("session %" PRIu64 " failed to read rpc header from socket %s:%u "
                              ...
                              session_id_, cached_address_.c_str(), cached_port_, err.value(), err.message().c_str(),
                              self.use_count());
                         this->stop();
                         return;
                     }
    ...
    void stop() {
        ...
        close_socket();
        if (callback_) { callback_(this->shared_from_this()); }
        handler_.reset();
    }
...
private:
    ...
    void read_complete(std::shared_ptr< buffer > hdr, std::shared_ptr< buffer > log_ctx) {
        ...
        try {
            ...
            if (hdr->get_int() > 0 && log_ctx) {
                ...
                if (flags_ & INCLUDE_LOG_TIMESTAMP) { LOG_ENTRY_SIZE += 8; }
    ...
    void on_resp_ready(std::shared_ptr< req_msg > req, std::shared_ptr< resp_msg > resp) {
        ...
        try {
            ...
            if (flags & INCLUDE_META) { bs.put_str(resp_meta_str); }
            ...
            aa::write(ssl_enabled_, ssl_socket_, socket_, asio::buffer(resp_buf->data_begin(), resp_buf->size()),
...
public:
    ...
    bool verify_certificate(bool preverified, asio::ssl::verify_context& ctx) {
        if (impl_->get_options().verify_sn_) {
            ...
            if (!impl_->get_options().verify_sn_(subject_name)) { return false; }
        }
        return preverified;
    }
#endif
    ...
    void send_retry(std::shared_ptr< asio_rpc_client > self, std::shared_ptr< asio::steady_timer > timer,
        ...
        if (err || num_send_fails_ >= SEND_RETRY_MAX) {
            if (err) {
                ...
                p_er("connection to %s:%s timeout (SSL %s)", host_.c_str(), port_.c_str(),
    ...
    virtual void send(std::shared_ptr< req_msg >& req, rpc_handler& when_done,
        ...
        while (!socket().is_open()) { // Dummy one-time loop
            ...
            if (!attempting_conn_.compare_exchange_strong(exp, desired)) {
                ...
                timer->async_wait(std::bind(&asio_rpc_client::send_retry, this, self, timer, req, when_done,
                                            send_timeout_ms, std::placeholders::_1));
                return;
            }
            ...
            if (impl_->get_options().custom_resolver_) {
                impl_->get_options().custom_resolver_(
                    ...
                    [this, self, req, when_done, send_timeout_ms](
                        ...
                        if (!err) {
                            p_in("custom resolver: %s:%s to %s:%s", host_.c_str(), port_.c_str(), resolved_host.c_str(),
                                 resolved_port.c_str());
        ...
        if (ssl_enabled_ && !ssl_ready_) {
            ...
            p_wn("cannot send req as SSL is not ready yet (%s:%s), count %zu", host_.c_str(), port_.c_str(),
        ...
        aa::write(ssl_enabled_, ssl_socket_, socket_, asio::buffer(req_buf->data(), req_buf->size()),
                  std::bind(&asio_rpc_client::sent, self, req, req_buf, when_done, std::placeholders::_1,
                            std::placeholders::_2));
    }
...
private:
    void execute_resolver(std::shared_ptr< asio_rpc_client > self, std::shared_ptr< req_msg > req,
        ...
        resolver_.async_resolve(
            q,
            [self, this, req, when_done, host, port, send_timeout_ms](std::error_code err,
                                                                      asio::ip::tcp::resolver::iterator itor) -> void {
                if (!err) {
                    asio::async_connect(socket(), itor,
        ...
        if (!err) {
            ...
            if (ssl_enabled_) {
                ...
                ssl_socket_.async_handshake(asio::ssl::stream_base::client,
                                            std::bind(&asio_rpc_client::handle_handshake, this, req, when_done,
                                                      send_timeout_ms, std::placeholders::_1));
#endif
            } else {
    ...
    void handle_handshake(std::shared_ptr< req_msg >& req, rpc_handler& when_done, uint64_t send_timeout_ms,
        ...
        if (!err) {
            ...
            abandoned_ = true;
            p_er("failed SSL handshake with peer %d, %s:%s, error %d, %s", req->get_dst(), host_.c_str(), port_.c_str(),
    ...
    void ctx_read(std::shared_ptr< req_msg >& req, std::shared_ptr< resp_msg >& rsp, rpc_handler& when_done,
        ...
        if (flags & INCLUDE_META) {
            ...
            if (impl_->get_options().read_resp_meta_ &&
                ...
                bool meta_ok = handle_custom_resp_meta(req, rsp, when_done,
...
asio_service_impl::asio_service_impl(const asio_service::options& _opt, std::shared_ptr< logger > l) :
    ...
    if (!cpu_cnt) { cpu_cnt = std::thread::hardware_concurrency(); }
...
void asio_service_impl::worker_entry() {
    ...
    if (my_opt_.worker_start_) { my_opt_.worker_start_(worker_id); }
    ...
    do {
        try {
            ...
            p_er("asio worker thread got exception: %s, "
                 ...
                 "stopping status %u",
                 ee.what(), num_active_workers_.load(), exception_count.load(), stopping_status_.load());
            // LCOV_EXCL_STOP
        }
    ...
    if (my_opt_.worker_stop_) { my_opt_.worker_stop_(worker_id); }
...
void asio_service::schedule(std::shared_ptr< delayed_task >& task, int32_t milliseconds) {
    ...
    timer->expires_after(
        std::chrono::duration_cast< std::chrono::nanoseconds >(std::chrono::milliseconds(milliseconds)));
