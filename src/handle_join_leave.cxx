...
std::shared_ptr<resp_msg> raft_server::handle_add_srv_req(req_msg& req) {
    ...
    std::shared_ptr<resp_msg> resp = std::make_shared<resp_msg>(
        state_->get_term(), msg_type::add_server_response, id_, leader_);
    ...
    if (srv_to_join_) {
        ...
        if (last_active_ms <= (uint64_t)raft_server::raft_limits_.response_limit_
...
void raft_server::sync_log_to_new_srv(uint64_t start_idx) {
    ...
    uint64_t gap =
        (quick_commit_index_ > start_idx) ? (quick_commit_index_ - start_idx) : 0;
    ...
    if (/* start_idx > 0 && */ start_idx < log_store_->start_index()) {
        ...
        req = std::make_shared<req_msg>(state_->get_term(),
...
std::shared_ptr<resp_msg> raft_server::handle_log_sync_req(req_msg& req) {
    std::vector<std::shared_ptr<log_entry>>& entries = req.log_entries();
    std::shared_ptr<resp_msg> resp(std::make_shared<resp_msg>(state_->get_term(),
