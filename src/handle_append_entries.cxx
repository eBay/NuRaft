...
bool raft_server::request_append_entries(std::shared_ptr<peer> p) {
    ...
    if (params->auto_adjust_quorum_for_small_cluster_ && get_num_voting_members() == 2
        ...
        if (cur_quorum_size >= 1) {
            ...
            if (do_adjustment) {
                std::shared_ptr<raft_params> clone =
                    std::make_shared<raft_params>(*params);
...
std::shared_ptr<req_msg>
raft_server::create_append_entries_req(std::shared_ptr<peer>& pp) {
    peer& p = *pp;
    ...
    if (!entries_valid) {
        ...
        std::shared_ptr<log_entry> custom_noti_le = std::make_shared<log_entry>(
            0, custom_noti->serialize(), log_val_type::custom);
...
std::shared_ptr<resp_msg> raft_server::handle_append_entries(req_msg& req) {
    ...
    if (req.get_term() < state_->get_term() || log_okay == false) {
        p_lv(log_lv,
             "deny, req term %" PRIu64 ", my term %" PRIu64 ", req log idx %" PRIu64
             ", my log idx %" PRIu64,
             req.get_term(),
             state_->get_term(),
             req.get_last_log_idx(),
    ...
    if (req.log_entries().size() > 0) {
        ...
        if (my_last_log_idx >= log_idx && cnt < req.log_entries().size()) {
            p_in("rollback logs: %" PRIu64 " - %" PRIu64 ", commit idx req %" PRIu64
                 ", quick %" PRIu64 ", sm %" PRIu64 ", "
...
bool raft_server::try_update_precommit_index(uint64_t desired,
