...
void peer::send_req(ptr< peer > myself, ptr< req_msg >& req, rpc_handler& handler) {
    ...
    rpc_handler h = (rpc_handler)std::bind(&peer::handle_rpc_result, this, myself, rpc_local, req, pending,
                                           std::placeholders::_1, std::placeholders::_2);
    if (rpc_local) { rpc_local->send(req, h); }
}
...
void peer::handle_rpc_result(ptr< peer > myself, ptr< rpc_client > my_rpc_client, ptr< req_msg >& req,
    ...
    std::unordered_set< int > msg_types_to_free({msg_type::append_entries_request, msg_type::install_snapshot_request,
                                                 ...
                                                 msg_type::reconnect_request, msg_type::priority_change_request});
    ...
    if (req) {
        p_tr("resp of req %d -> %d, type %s, %s", req->get_src(), req->get_dst(),
    ...
    if (err == nilptr) {
        // Succeeded.
        {
            ...
            if (msg_types_to_free.find(req->get_type()) != msg_types_to_free.end()) { set_free(); }
        }
...
bool peer::recreate_rpc(ptr< srv_config >& config, context& ctx) {
    ...
    if (backoff_timer_disabled) { p_tr("reconnection back-off timer is disabled"); }
