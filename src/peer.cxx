...
void peer::send_req(std::shared_ptr<peer> myself,
...
void peer::handle_rpc_result(std::shared_ptr<peer> myself,
...
bool peer::recreate_rpc(std::shared_ptr<srv_config>& config, context& ctx) {
    ...
    if (backoff_timer_disabled || reconn_backoff_.timeout()) {
        ...
        new_duration_ms =
            std::min(static_cast<uint64_t>(hb_interval_), new_duration_ms * 2);
