...
void raft_server::request_prevote() {
    ...
    if (pre_vote_.live_ + pre_vote_.dead_ > 0) {
        if (pre_vote_.live_ + pre_vote_.dead_ < quorum_size + 1) {
            ...
            p_wn("total %d nodes (including this node) responded for pre-vote "
                 ...
                 pre_vote_.live_.load() + pre_vote_.dead_.load(), pre_vote_.term_, pre_vote_.live_.load(),
                 pre_vote_.dead_.load(), quorum_size + 1, pre_vote_.failure_count_.load());
        } else {
    ...
    p_in("[PRE-VOTE INIT] my id %d, my role %s, term %" PRIu64 ", log idx %" PRIu64 ", "
         ...
         id_, srv_role_to_string(role_).c_str(), state_->get_term(), log_store_->next_slot() - 1,
...
void raft_server::initiate_vote(bool force_vote) {
    ...
    if (!force_vote && grace_period && sm_commit_index_ < lagging_sm_target_index_) {
        p_in("grace period option is enabled, and state machine needs catch-up: "
             "%" PRIu64 " vs. %" PRIu64 "",
             sm_commit_index_.load(), lagging_sm_target_index_.load());
        if (vote_init_timer_term_ != cur_term) {
            p_in("grace period: %d, term increment detected %" PRIu64 " vs. %" PRIu64 ", reset timer", grace_period,
                 vote_init_timer_term_.load(), cur_term);
        ...
        if (vote_init_timer_term_ == cur_term && !vote_init_timer_.timeout()) {
            ...
            p_in("grace period: %d, term %" PRIu64 ", waited %" PRIu64 " ms, skip initiating vote", grace_period,
                 cur_term, vote_init_timer_.get_ms());
            return;

        } else {
            p_in("grace period: %d, no new leader detected for term %" PRIu64 " for %" PRIu64 " ms", grace_period,
                 cur_term, vote_init_timer_.get_ms());
        }
    }
...
void raft_server::request_vote(bool force_vote) {
    ...
    for (peer_itor it = peers_.begin(); it != peers_.end(); ++it) {
        ...
        p_db("send %s to server %d with term %" PRIu64 "", msg_type_to_string(req->get_type()).c_str(),
...
std::shared_ptr< resp_msg > raft_server::handle_vote_req(req_msg& req) {
    p_in("[VOTE REQ] my role %s, from peer %d, "
         ...
         srv_role_to_string(role_).c_str(), req.get_src(), req.get_last_log_term(),
        ...
        for (auto& entry : c_conf->get_servers()) {
            ...
            if (!ignore_priority && s_conf->get_id() == req.get_src() && s_conf->get_priority() &&
                ...
                p_in("I (%d) could vote for peer %d, "
                     ...
                     id_, s_conf->get_id(), s_conf->get_priority(), target_priority_);
                p_in("decision: X (deny)\n");
                return resp;
            }
        }

        p_in("decision: O (grant), voted_for %d, term %" PRIu64, req.get_src(), resp->get_term());
...
void raft_server::handle_vote_resp(resp_msg& resp) {
    ...
    if (resp.get_accepted()) { votes_granted_ += 1; }
    ...
    p_in("[VOTE RESP] peer %d (%s), resp term %" PRIu64 ", my role %s, "
         ...
         resp.get_src(), (resp.get_accepted()) ? "O" : "X", resp.get_term(), srv_role_to_string(role_).c_str(),
         (int)votes_granted_, (int)votes_responded_, get_num_voting_members(), election_quorum_size);
...
std::shared_ptr< resp_msg > raft_server::handle_prevote_req(req_msg& req) {
    ...
    //   requests.
    if (catching_up_) { p_in("this server is catching up, always accept pre-vote"); }
...
void raft_server::handle_prevote_resp(resp_msg& resp) {
    if (resp.get_term() != pre_vote_.term_) {
        ...
        p_in("[PRE-VOTE RESP] from peer %d, my role %s, "
             ...
             "ignore it.",
             resp.get_src(), srv_role_to_string(role_).c_str(), resp.get_term(), pre_vote_.term_);
        return;
    }
    ...
    p_in("[PRE-VOTE RESP] peer %d (%s), term %" PRIu64 ", resp term %" PRIu64 ", "
         ...
         "num voting members %d, quorum %d\n",
         resp.get_src(), (resp.get_accepted()) ? "O" : "X", pre_vote_.term_, resp.get_term(),
         srv_role_to_string(role_).c_str(), pre_vote_.dead_.load(), pre_vote_.live_.load(), pre_vote_.abandoned_.load(),
    ...
    if (pre_vote_.live_ >= election_quorum_size) {
        ...
        p_wn("[PRE-VOTE] rejected by quorum, count %d", pre_vote_.quorum_reject_count_.load());
