...
std::shared_ptr< cmd_result< std::shared_ptr< buffer > > > raft_server::remove_srv(const int srv_id) {
...
raft_server::append_entries_ext(const std::vector< std::shared_ptr< buffer > >& logs,
    ...
    for (auto it = logs.begin(); it != logs.end(); ++it) {
        ...
        buf->pos(0);
        std::shared_ptr< log_entry > log(std::make_shared< log_entry >(0, *it, log_val_type::app_log));
        req->log_entries().push_back(log);
    }
...
raft_server::send_msg_to_leader(std::shared_ptr< req_msg >& req, const req_ext_params& ext_params) {
    ...
    if (leader_id == -1) {
        ...
        std::shared_ptr< cmd_result< std::shared_ptr< buffer > > > ret =
            std::make_shared< cmd_result< std::shared_ptr< buffer > > >(result, cmd_result_code::NOT_LEADER);
        return ret;
    }
    ...
    if (leader_id == id_) {
        ...
        if (!resp) {
            ...
            std::shared_ptr< cmd_result< std::shared_ptr< buffer > > > ret =
                std::make_shared< cmd_result< std::shared_ptr< buffer > > >(result, cmd_result_code::BAD_REQUEST);
            return ret;
        }
        ...
        if (result) { result->pos(0); }

        if (!ret) {
            // In blocking mode,
            // we already have result when we reach here.
            ret = std::make_shared< cmd_result< std::shared_ptr< buffer > > >(result, resp->get_accepted(),
    ...
    do {
        ...
        if (e_rpc == cur_pkg->rpc_client_idle_.end()) {
            ...
            p_tr("no connection available, idle %zu, in-use %zu, max %zu", cur_pkg->rpc_client_idle_.size(),
    ...
    if (!rpc_cli) { return std::make_shared< cmd_result< std::shared_ptr< buffer > > >(result); }

    std::shared_ptr< cmd_result< std::shared_ptr< buffer > > > presult(
        std::make_shared< cmd_result< std::shared_ptr< buffer > > >());

    rpc_handler handler = std::bind(&raft_server::auto_fwd_resp_handler, this, presult, cur_pkg, rpc_cli,
                                    std::placeholders::_1, std::placeholders::_2);
    rpc_cli->send(req, handler, params->auto_forwarding_req_timeout_);

    if (params->return_method_ == raft_params::blocking) { presult->get(); }
...
void raft_server::auto_fwd_release_rpc_cli(std::shared_ptr< auto_fwd_pkg > cur_pkg,
    ...
    if (is_blocking_mode) {
        ...
        if (!auto_fwd_reqs_.empty()) {
            ...
            rpc_handler handler = std::bind(&raft_server::auto_fwd_resp_handler, this, entry.resp, cur_pkg, rpc_cli,
                                            std::placeholders::_1, std::placeholders::_2);
...
void raft_server::auto_fwd_resp_handler(std::shared_ptr< cmd_result< std::shared_ptr< buffer > > > presult,
                                        std::shared_ptr< auto_fwd_pkg > cur_pkg, std::shared_ptr< rpc_client > rpc_cli,
