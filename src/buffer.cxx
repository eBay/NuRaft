...
#define __size_of_block(p) \
    (__is_big_block(p)) ? (*((uint32_t*)(p)) ^ 0x80000000) : *((uint16_t*)(p))
...
#define __entire_data_of_block(p)                                \
    (__is_big_block(p)) ? (byte*)((byte*)(((uint32_t*)(p)) + 2)) \
                        : (byte*)((byte*)(((uint16_t*)(p)) + 2))
...
std::shared_ptr<buffer> buffer::alloc(const size_t size) {
    ...
    if (size >= 0x8000) {
        ...
        std::shared_ptr<buffer> buf(reinterpret_cast<buffer*>(new char[len]),
                                    &free_buffer);
        amount_allocs += len;
        amount_active += len;
...
size_t buffer::container_size() const {
    return (size_t)(
        __size_of_block(this)
        + ((__is_big_block(this)) ? sizeof(uint32_t) * 2 : sizeof(uint16_t) * 2));
}
...
int32_t buffer::get_int() {
    ...
    if (avail < sz_int) {
        throw std::overflow_error("insufficient buffer available for an int32_t value");
    }
...
uint64_t buffer::get_uint64() {
    size_t avail = size() - pos();
    if (avail < sz_uint64_t) {
        throw std::overflow_error("insufficient buffer available for an uint64_t value");
    }
...
const byte* buffer::get_bytes(size_t& len) {
    ...
    if (avail < sz_int) {
        throw std::overflow_error(
            "insufficient buffer available for a bytes length (int32_t)");
    }
...
void buffer::put(const byte* ba, size_t len) {
    ...
    if (size() - pos() < sz_int) {
        throw std::overflow_error("insufficient buffer to store int32_t length");
    }
...
void buffer::put(int32_t val) {
    if (size() - pos() < sz_int) {
        throw std::overflow_error("insufficient buffer to store int32_t");
    }
