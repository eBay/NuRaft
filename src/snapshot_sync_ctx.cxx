...
snapshot_sync_ctx::snapshot_sync_ctx(const std::shared_ptr<snapshot>& s,
                                     int peer_id,
                                     uint64_t timeout_ms,
                                     uint64_t offset)
    : peer_id_(peer_id)
...
struct snapshot_io_mgr::io_queue_elem {
    io_queue_elem(std::shared_ptr<raft_server> r,
                  std::shared_ptr<snapshot> s,
                  std::shared_ptr<snapshot_sync_ctx> c,
    ...
    std::function<void(std::shared_ptr<resp_msg>&, std::shared_ptr<rpc_exception>&)>
        handler_;
};
...
bool snapshot_io_mgr::push(
    std::shared_ptr<raft_server> r,
    std::shared_ptr<peer> p,
