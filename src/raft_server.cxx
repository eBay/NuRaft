...
raft_server::raft_server(context* ctx)
    : raft_server(ctx, init_options::__default_options) {}
...
raft_server::raft_server(context* ctx, raft_server::init_options const& opt)
    ...
    for (auto it = c_conf->get_servers().begin(); it != srvs_end; ++it) {
        ...
        peer_info_msg << "peer " << cur_srv->get_id() << ": DC ID "
                      << cur_srv->get_dc_id() << ", " << cur_srv->get_endpoint() << ", "
                      << (cur_srv->is_learner() ? "learner" : "voting member") << ", "
                      << cur_srv->get_priority() << std::endl;
    }

    peer_info_msg << "my id: " << id_ << ", "
                  << ((im_learner_) ? "learner" : "voting_member") << std::endl;
    peer_info_msg << "num peers: " << peers_.size() << std::endl;
    p_in("%s", peer_info_msg.str().c_str());

    if (opt._start_server_in_constructor) {
        start_server(opt._skip_initial_election_timeout);
    }
}
...
std::shared_ptr<resp_msg> raft_server::process_req(req_msg& req,
...
void raft_server::reset_peer_info() {
    ...
    if (srv_cnt > 1) {
        ...
        std::shared_ptr<cluster_config> my_next_config = std::make_shared<cluster_config>(
        ...
        std::shared_ptr<log_entry> entry(
            std::make_shared<log_entry>(state_->get_term(),
                                        new_conf_buf,
                                        log_val_type::conf,
...
void raft_server::handle_peer_resp(std::shared_ptr<resp_msg>& resp,
...
std::shared_ptr<resp_msg> raft_server::handle_reconnect_req(req_msg& req) {
    ...
    std::shared_ptr<resp_msg> resp(std::make_shared<resp_msg>(
        state_->get_term(), msg_type::reconnect_response, id_, srv_id));
    if (role_ != srv_role::leader) {
...
void raft_server::become_leader() {
    ...
    std::shared_ptr<raft_params> params = ctx_->get_params();
    {
        ...
        std::shared_ptr<cluster_config> last_config_cloned =
...
bool raft_server::request_leadership() {
    ...
    std::shared_ptr<req_msg> req =
        std::make_shared<req_msg>(state_->get_term(),
...
void raft_server::handle_ext_resp(std::shared_ptr<resp_msg>& resp,
                                  std::shared_ptr<rpc_exception>& err) {
    auto guard = recur_lock(lock_);
    if (err) {
...
void raft_server::on_retryable_req_err(std::shared_ptr<peer>& p,
                                       std::shared_ptr<req_msg>& req) {
...
void raft_server::set_user_ctx(const std::string& ctx) {
    ...
    std::shared_ptr<cluster_config> cloned_config =
        cluster_config::deserialize(*enc_conf);
