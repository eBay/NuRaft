...
inmem_log_store::~inmem_log_store() {
    if (disk_emul_thread_) {
        ...
        if (disk_emul_thread_->joinable()) { disk_emul_thread_->join(); }
    }
}
...
ulong inmem_log_store::last_durable_index() {
    ...
    if (!disk_emul_delay) { return last_log; }
...
void inmem_log_store::disk_emul_loop() {
    ...
    while (!disk_emul_thread_stop_signal_) {
        ...
        bool call_notification = false;
        {
            ...
            if (entry != disk_emul_logs_being_written_.end()) { next_sleep_us = entry->first - cur_time; }
        }

        if (call_notification) { raft_server_bwd_pointer_->notify_log_append_completion(true); }
    }
}
